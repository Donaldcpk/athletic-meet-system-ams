...
class RecordsService {
  ...
  static void initializeRecords() {
    _records.clear();
    _parseRecordsFromCSV();
  }

  /// 解析CSV數據
  static void _parseRecordsFromCSV() {
    // 模擬CSV數據解析 (實際應從檔案讀取)
    final csvData = [
      // 男子丙組 60m
      ...
      // 接力項目
      ['4X100m', '4X100m', 'S1-2', '', '2C', '09-10', '51.66s', '58.50s'],
      ['4X100m', '4X100m', 'S3-4', '', '4D', '13-14', '49.54s', '54.50s'],
      ['4X100m', '4X100m', 'S5-6', '', '5B', '16-17', '50.67s', '52.50s'],
      
      ['4X400m', '4X400m', 'S1-2', '', '2C', '10-11', '4:25.32', '5:17.00'],
      ['4X400m', '4X400m', 'S3-4', '', '4F', '12-13', '4:00.14', '4:59.00'],
      ['4X400m', '4X400m', 'S5-6', '', '6C', '11-12', '3:59.26', '4:48.00'],
    ];
    ...
    print('✅ 已載入 ${_records.length} 項校內紀錄和標準成績');
  }

  /// 解析單行紀錄
  static void _parseRecord(List<String> row) {
    if (row.length < 8) return;
    
    final eventCode = row[0];
    final eventName = row[1];
    final grade = row[2];
    final recordHolder = row[3].isNotEmpty ? row[3] : null;
    final recordClass = row[4].isNotEmpty ? row[4] : null;
    final recordYear = row[5].isNotEmpty ? row[5] : null;
    final recordValue = row[6].isNotEmpty ? row[6] : null;
    final standardValue = row[7].isNotEmpty ? row[7] : null;
    ...
    if (eventCode.startsWith('B')) {
      gender = Gender.male;
      if (eventCode.contains('A')) division = Division.senior;
      else if (eventCode.contains('B')) division = Division.junior;
      else if (eventCode.contains('C')) division = Division.primary;
    ...
    if (gender == null || division == null) return;
    ...
    _records[eventCode] = record;
  }

  /// 獲取項目紀錄
  static EventRecord? getEventRecord(String eventCode) {
    return _records[eventCode];
  }

  /// 獲取匹配的紀錄 (根據項目名稱、性別、組別)
  static EventRecord? getMatchingRecord(String eventName, Gender gender, Division division) {
    // 構建可能的事件代碼
    String genderPrefix = gender == Gender.male ? 'B' : 'G';
    String divisionSuffix = division == Division.senior ? 'A' : 
                           (division == Division.junior ? 'B' : 'C');
    String eventCode = '$genderPrefix$divisionSuffix';
    
    // 先嘗試精確匹配
    final exactMatch = _records[eventCode];
    if (exactMatch != null && exactMatch.eventName == eventName) {
      return exactMatch;
    }
  ...
  /// 檢查成績是否達到標準
  static bool meetsStandard(String eventName, Gender gender, Division division, String result) {
    final record = getMatchingRecord(eventName, gender, division);
    return record?.meetsStandard(result) ?? false;
  }

  /// 檢查成績是否破校紀錄
  static bool breaksRecord(String eventName, Gender gender, Division division, String result) {
    final record = getMatchingRecord(eventName, gender, division);
    return record?.breaksRecord(result) ?? false;
  }

  /// 獲取所有紀錄
  static Map<String, EventRecord> get allRecords => Map.from(_records);
